  ...
  return (
    <section className="px-6 sm:px-10 py-32 max-w-6xl mx-auto">
      ...
      <div ref={r2} className="reveal glass-solar pl-5 pr-5 py-5 mb-16 max-w-2xl rounded-lg" style={{ borderLeft: '2px solid oklch(80% 0.18 80)' }}>
        ...
        <p className="text-[15px] mb-1.5" style={{ color: 'oklch(90% 0.005 270)', lineHeight: 1.7 }}>
          <strong style={{ color: 'oklch(92% 0.004 270)' }}>2pc</strong> {bisGear.tierSet.twoPiece}
        </p>
        <p className="text-[15px]" style={{ color: 'oklch(90% 0.005 270)', lineHeight: 1.7 }}>
          <strong style={{ color: 'oklch(92% 0.004 270)' }}>4pc</strong> {bisGear.tierSet.fourPiece}
        </p>
      </div>
      ...
      <div className="glass-lunar pl-5 pr-5 py-4 mb-16 max-w-2xl rounded-lg" style={{ borderLeft: '2px solid oklch(68% 0.16 285)' }}>
        <h4 className="text-[12px] uppercase font-bold mb-2" style={{ color: 'oklch(68% 0.16 285)', letterSpacing: '0.1em' }}>
          AoE Stat Priority (Elune's Chosen)
        </h4>
        <p className="text-[15px] font-semibold" style={{ color: 'oklch(82% 0.01 270)' }}>
          Intellect {'>'} <span style={{ color: 'oklch(80% 0.18 80)' }}>Haste</span> {'>'} Mastery {'>'} Crit {'>'} Versatility
        </p>
        <p className="text-[14px] mt-1" style={{ color: 'oklch(58% 0.012 270)' }}>
      ...
      <div ref={r3} className="reveal rounded-lg overflow-hidden mb-16" style={{ border: '1px solid oklch(14% 0.01 270)' }}>
        ...
        {bisGear.raidMythic.map((item, i) => (
          <div key={item.slot}
            ...
            <div className="col-span-2 text-[14px]" style={{ color: 'oklch(90% 0.005 55)' }}>{item.slot}</div>
            <div className="col-span-6 text-[15px] font-semibold" style={{ color: item.isTier ? 'oklch(80% 0.18 80)' : 'oklch(88% 0.006 270)' }}>
              {item.name}
            </div>
            <div className="col-span-4 text-[14px] hidden sm:block" style={{ color: 'oklch(52% 0.012 270)' }}>{item.source}</div>
          </div>
        ))}
      </div>
      ...
      <div ref={r4} className="reveal">
          ...
          {bisGear.trinkets.rankings.slice(0, 2).map(t => (
            <div key={t.name} className={`p-5 rounded-lg card-hover ${t.type === 'On-Use' ? 'glass-solar' : 'glass'}`}>
              <div className="flex items-baseline justify-between mb-2">
                ...
                <span className="text-[12px] font-semibold" style={{ color: 'oklch(52% 0.012 270)' }}>{t.type}</span>
              </div>
              <p className="text-[14px]" style={{ color: 'oklch(52% 0.012 270)', lineHeight: 1.6 }}>{t.notes}</p>
            </div>
          ))}
        </div>
            ...
            {bisGear.trinkets.rankings.slice(2).map(t => (
              <div key={t.name} className="flex items-baseline justify-between px-4 py-2.5 rounded"
                ...
                <div className="flex items-baseline gap-3">
                  <span className="text-[15px] font-semibold" style={{ color: 'oklch(78% 0.008 270)' }}>{t.name}</span>
                  <span className="text-[13px]" style={{ color: 'oklch(52% 0.012 270)' }}>{t.type}</span>
                </div>
                <span className="text-[13px]" style={{ color: 'oklch(52% 0.012 270)' }}>{t.source}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="grid sm:grid-cols-2 gap-12">
          <div>
            <div className="text-[11px] uppercase font-bold mb-3" style={{ color: 'oklch(52% 0.012 270)', letterSpacing: '0.12em' }}>Weapon</div>
            <p className="text-sm font-semibold mb-1" style={{ color: 'oklch(88% 0.006 270)' }}>{bisGear.weapon.best}</p>
            <p className="text-[14px]" style={{ color: 'oklch(90% 0.005 55)', lineHeight: 1.6 }}>{bisGear.weapon.notes}</p>
          </div>
          <div>
            <div className="text-[11px] uppercase font-bold mb-3" style={{ color: 'oklch(52% 0.012 270)', letterSpacing: '0.12em' }}>Embellishments</div>
            <p className="text-sm font-semibold mb-0.5" style={{ color: 'oklch(88% 0.006 270)' }}>{bisGear.embellishments.first.name}</p>
            <p className="text-[14px] mb-3" style={{ color: 'oklch(90% 0.005 55)', lineHeight: 1.6 }}>{bisGear.embellishments.first.notes}</p>
            <p className="text-sm font-semibold mb-0.5" style={{ color: 'oklch(88% 0.006 270)' }}>{bisGear.embellishments.second.name}</p>
            <p className="text-[14px]" style={{ color: 'oklch(90% 0.005 55)', lineHeight: 1.6 }}>{bisGear.embellishments.second.notes}</p>
          </div>
        </div>
      </div>
    </section>
