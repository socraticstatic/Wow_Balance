        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-6">
          {affixTiers.map(a => (
            <div key={a.level} className="p-3 rounded-lg glass">
              ...
              <div className="text-[11px]" style={{ color: 'oklch(72% 0.01 270)' }}>{a.desc}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-2">
          {bargainTips.map(b => (
            <div key={b.name} className="flex items-start gap-2 p-3 rounded-lg" style={{ background: 'oklch(9% 0.008 45)', border: '1px solid oklch(14% 0.01 45)' }}>
              ...
              <span className="text-[12px]" style={{ color: 'oklch(74% 0.01 270)' }}>{b.tip}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="reveal mb-12 flex flex-wrap gap-2">
        {[...dungeons].sort((a, b) => b.balanceRating - a.balanceRating).map(d => (
          ...
          >
            ...
            <div className="text-[9px]" style={{ color: 'oklch(58% 0.01 50)' }}>{d.timer}</div>
          </button>
        ))}
      </div>
      ...
      <div className="rounded-lg overflow-hidden glass">
        ...
        >
          <div className="flex items-baseline gap-3">
            ...
            <span className="text-[10px] font-medium" style={{ color: 'oklch(58% 0.01 50)' }}>{dungeon.expansion}</span>
            <span className="font-mono text-[10px]" style={{ color: 'oklch(54% 0.01 50)', fontVariantNumeric: 'tabular-nums' }}>{dungeon.timer}</span>
          </div>
          <div className="flex items-center gap-4">
            ...
            <span style={{ color: 'oklch(58% 0.01 50)', transform: isExpanded ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s ease', display: 'inline-block' }}>
              &#8963;
            </span>
          </div>
        </button>
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-6">
              <div>
                ...
                <ul className="space-y-2 mb-6">
                  {dungeon.keyTrash.map((t, i) => (
                    <li key={i} className="text-[13px]" style={{ color: 'oklch(76% 0.01 270)', lineHeight: 1.7 }}>{t}</li>
                  ))}
                </ul>
                ...
                <ul className="space-y-2">
                  {dungeon.bossNotes.map((b, i) => (
                    <li key={i} className="text-[13px]" style={{ color: 'oklch(76% 0.01 270)', lineHeight: 1.7 }}>{b}</li>
                  ))}
                </ul>
              </div>
              ...
              <div>
                ...
                <div className="p-4 rounded-lg" style={{ background: 'oklch(9% 0.008 45)', border: '1px solid oklch(14% 0.01 45)' }}>
                  <div className="text-[9px] uppercase font-bold mb-2" style={{ color: 'oklch(72% 0.01 50)', letterSpacing: '0.12em' }}>
                    Utility Checklist
                  </div>
                  <ul className="space-y-1">
                    {dungeon.utility.map(u => (
                      <li key={u} className="flex items-center gap-2 text-[12px]" style={{ color: 'oklch(74% 0.01 270)' }}>
